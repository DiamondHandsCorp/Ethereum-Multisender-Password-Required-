...
contract TokenMultiSend {
    ...
    function multiSend(address[] memory _tokenAddresses, address[] memory _addresses, uint256[] memory _amounts, uint256 _nonce, string memory _password) public {
        ...
        for (uint256 i = 0; i < _tokenAddresses.length; i++) {
            require(_amounts[i] >= 0, "Invalid amount"); // Input validation check
            IERC20 token = IERC20(_tokenAddresses[i]);
            require(token.transferFrom(msg.sender, _addresses[i], _amounts[i]), "Transfer failed");
            totalAmount += _amounts[i];
        }

        require(totalAmount > 0, "Total amount must be greater than 0"); // Input validation check
        uint256 fee = totalAmount / 40; // 2.5% fee
        IERC20 tokenFee = IERC20(_tokenAddresses[0]); // Assume first token in the list is used for the fee
        require(tokenFee.transfer(owner, fee), "Fee transfer failed");
